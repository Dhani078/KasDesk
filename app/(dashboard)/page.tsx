  ...
  return (
    <main className="mx-auto min-h-dvh w-full max-w-2xl px-5 pt-7 pb-32 sm:px-8 sm:pt-10">
      <header className="mb-5">
        <div className="mb-4 flex items-center justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-accent">{greeting}</p>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <PrivacyToggle />
            <Link href="/settings" aria-label="Pengaturan" className="grid h-11 w-11 place-items-center rounded-xl border border-border-outer bg-surface text-text-secondary transition hover:border-accent/35 hover:text-text-primary active:scale-95"><Settings className="h-4 w-4" aria-hidden /></Link>
            <LogoutButton />
          </div>
        </div>

        {!hasAnyData && <section className="mb-4 rounded-3xl border border-accent/25 bg-gradient-to-br from-accent/15 to-accent/5 p-5"><div className="flex gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-accent/15 text-accent"><Sparkles className="h-5 w-5" aria-hidden /></span><div><h1 className="text-lg font-semibold text-text-primary">Mulai dari dompet pertama</h1><p className="mt-1 text-sm leading-6 text-text-secondary">Tambahkan cash, bank, atau e-wallet. Setelah itu dashboard otomatis menampilkan Aman Harian, coach, dan laporan.</p><Link href="/wallets" className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-accent-solid px-4 text-sm font-semibold text-white"><PlusCircle className="h-4 w-4" aria-hidden /> Buat dompet</Link></div></div></section>}

        <section className="surface-card relative overflow-hidden rounded-[2rem] border border-border-outer p-5 shadow-sm sm:p-6">
          <div className="pointer-events-none absolute -right-12 -top-16 h-40 w-40 rounded-full bg-accent/15 blur-3xl" aria-hidden />
          <div className="relative">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div><p className="text-xs uppercase tracking-[0.1em] text-text-secondary">Total saldo</p><p className="mt-1 text-xs text-text-secondary">Semua dompet aktif</p></div>
              <span className="rounded-full bg-accent/10 px-3 py-1 text-xs font-semibold text-accent">{dash.walletCount} Dompet</span>
            </div>
            <h1 className="font-mono text-4xl font-semibold tracking-tight tabular-nums text-text-primary sm:text-5xl">{formatIDR(dash.totalBalance)}</h1>
            <div className="mt-5 grid grid-cols-2 gap-2">
              <div className="rounded-2xl bg-white/[0.035] p-3"><p className="text-[11px] uppercase tracking-[0.08em] text-text-secondary">Aman / hari</p><p className="mt-1 font-mono text-sm font-semibold text-accent-income">{formatIDR(dash.safeDailySpend)}</p></div>
              <div className="rounded-2xl bg-white/[0.035] p-3"><p className="text-[11px] uppercase tracking-[0.08em] text-text-secondary">Bulan ini</p><p className={`mt-1 font-mono text-sm font-semibold ${netMonth >= 0 ? 'text-accent-income' : 'text-accent-expense'}`}>{formatIDR(netMonth)}</p></div>
            </div>
      ...
      <section className="mb-6 grid grid-cols-3 gap-2" aria-label="Aksi cepat">
        <Link href="/transactions" className="surface-card rounded-2xl p-3 text-center text-xs font-semibold text-text-primary transition hover:border-accent/35 active:scale-95"><PlusCircle className="mx-auto mb-2 h-5 w-5 text-accent" aria-hidden />Catat</Link>
        <Link href="/coach" className="surface-card rounded-2xl p-3 text-center text-xs font-semibold text-text-primary transition hover:border-accent/35 active:scale-95"><Sparkles className="mx-auto mb-2 h-5 w-5 text-accent" aria-hidden />Coach</Link>
        <Link href="/insights" className="surface-card rounded-2xl p-3 text-center text-xs font-semibold text-text-primary transition hover:border-accent/35 active:scale-95"><ArrowUpRight className="mx-auto mb-2 h-5 w-5 text-accent" aria-hidden />Laporan</Link>
      </section>

      <DailyFocus dash={dash} />

      <section className="surface-card mb-6 rounded-3xl p-5">
        <div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-accent/10 text-accent"><ShieldCheck className="h-5 w-5" aria-hidden /></span><div><p className="text-xs uppercase tracking-[0.08em] text-text-secondary">Skor kesehatan</p><h2 className="text-lg font-semibold text-text-primary">{dash.healthLabel}</h2></div></div><p className="font-mono text-3xl font-semibold text-accent">{dash.healthScore}</p></div>
        ...
        <Link href="/coach" className="mt-4 inline-flex min-h-10 items-center rounded-xl border border-border-outer px-3 text-xs font-semibold text-accent transition hover:border-accent/35">Lihat saran coach →</Link>
      </section>
