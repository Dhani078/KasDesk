...
export default async function InsightsPage() {
  const [flow, top, debts] = await Promise.all([getSpendingFlow(), getTopCategories(7), getDebts()])
        ...
        <div className="mt-6 flex h-40 items-end gap-2" role="img" aria-label="Grafik pengeluaran tujuh hari terakhir">
          {flow.map((item) => (
            <div key={item.day.toISOString()} className="flex min-w-0 flex-1 flex-col items-center gap-2">
              <div className="flex h-28 w-full items-end rounded-lg bg-white/[0.025] p-1">
                <div className="w-full rounded-md bg-gradient-to-t from-accent-solid to-accent transition-opacity hover:opacity-80" style={{ height: `${Math.max(4, (item.total / maxFlow) * 100)}%` }} title={`${formatDateShort(item.day)}: ${formatIDR(item.total)}`} />
              </div>
              <span className="text-xs text-text-secondary">{formatDateShort(item.day).split(' ')[0]}</span>
            </div>
          ))}
        </div>
