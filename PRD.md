...
| 11.25 | **Insight "7 Hari Terakhir" showed the month total** — heading said last 7 days but rendered `dash.monthlyExpense` (month-to-date), so the number was inflated by older spending and misstated burn rate. Fixed: sum `getSpendingFlow()`, which is already 7-day/expense-filtered; removed the now-unused `getDashboard()` query | `test:insights` (5/5) | Done | NFR-DATA-1 |
| 11.26 | **Archived wallets could silently absorb money** — `getWallets()` hides archived wallets (isArchived=0) so the UI never offers them, but `createTransaction()` did not reject them at the source. Since archived wallets are excluded from Total Saldo, any spend against one makes money vanish from every headline number with no visible cause. Fixed: `WALLET_ARCHIVED` thrown for both source and destination wallet, surfaced as a clear Indonesian message; rule extracted to `lib/wallet-guard.ts` so it is unit-testable without a Next request scope | `test:archived` (6/6) + `test:archived:unit` (12/12) | Done | NFR-DATA-2 |
| 11.27 | **Archived-wallet guard was incomplete** — the guard added for 11.26 covered only `createTransaction`, but two more paths move money: `depositToVault` (out of a wallet) and `withdrawFromVault` (into one). An archived DESTINATION is equally lossy: the balance rises while Total Saldo stays flat, so money still vanishes. Fixed: both now select `isArchived` and throw `WALLET_ARCHIVED`. `deleteTransaction` is deliberately EXEMPT — it reverses an existing transaction, and refusing would strand money in any archived wallet forever, since its old transactions could never be deleted. That exemption is asserted by name in the test so a future consistency refactor must confront it | `test:archived:coverage` (20/20) | Done | NFR-DATA-2 |
| 11.28 | **Registration was not atomic** — `registerUser` inserted the user row and then called `seedNewUser` (starter wallet + 9 system categories) as two separate, untransacted inserts. A failure between them created an account that could log in but had no wallet and no categories — permanently broken, since nothing ever re-runs the seed. Fixed: both now run in one `db.transaction()`; `seedNewUser` accepts an optional tx handle. Password hashing stays outside the transaction (bcrypt is slow and would hold a connection ~100ms) | `test:register:atomic` (4/4, proves both the old failure mode and the rollback) | Done | NFR-DATA-3 |
| 11.29 | **Test fixtures accumulated in the production database** — most test scripts create users directly through SQL, bypassing `registerUser()`, so those rows are never seeded and some survive a failed run. 7 of 8 users were debris, which made integrity scans look like real corruption. Fixed: `scripts/cleanup-fixtures.js` (report-only by default, `--apply` to delete) matches only `<prefix>-<ms-timestamp>@example.com`. Also rewrote `test:newuser-seed` to call `registerUser()` directly via tsx — it previously skipped, because the signup HTTP route does not exist, so the real path was never verified | `test:newuser-seed` (4/4) | Done | NFR-TEST-1 |
| 11.30 | **Wallet archiving was unreachable — a dangling feature** — the `isArchived` column existed, was filtered on in `getWallets()`, and was guarded in every money path (11.26, 11.27), but NO action or UI could ever set it. An unused wallet was therefore stuck in Total Saldo forever, with no way to retire it. Fixed: `archiveWallet(id, archived)` action (owner-scoped) plus archive/restore UI. Critically, `/wallets` now lists archived wallets in a separate section — archiving via `getWallets()` alone would be a ONE-WAY DOOR, hiding the wallet with no way back | `test:wallet-archive` (10/10, incl. restore + cross-user isolation) | Done | FR-WALLET-4 |
| 11.31 | **OCR upload buffered unbounded bodies before checking size** — `await req.formData()` reads the ENTIRE request into memory, and the 10 MB `file.size` check ran only afterwards. A multi-hundred-MB upload would therefore be buffered into RAM and only then rejected — a memory DoS, with 20 requests/min per user making it cheap to sustain. Fixed: a `Content-Length` pre-check before parsing (advisory, so the `file.size` backstop remains the real gate). Measured: an 11 MB upload is now refused 413 in ~139ms | `test:ocr:limits` (7/7) | Done | NFR-SEC-2 |
| 11.32 | **Gemini API key travelled in the URL query string** — the route called `fetch(\`${ENDPOINT}?key=${apiKey}\`)`. Query strings are recorded by proxy and CDN logs, and any error message echoing the request URL would carry the key with it — a leak the application's own logs would never show. Fixed: sent as the `x-goog-api-key` header, verified against the live endpoint (valid key -> 429 quota; invalid key -> 400 "API key not valid", so the control proves the header is actually read). Also confirmed the key is never logged or returned | `test:key-transport` (7/7, structural + live control) | Done | NFR-SEC-3 |
| 11.33 | **No security headers at all** — responses carried none of `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy` or `Permissions-Policy`. For a finance app the framing gap is the sharpest: a look-alike site could iframe the app for clickjacking. Fixed in `next.config.ts` for all routes. CSP shipped REPORT-ONLY (not enforcing) on purpose — a strict policy needs Next's inline bootstrap, the PWA service worker and Tailwind's runtime accounted for first, and a wrong enforcing CSP is worse than none. Test asserts both presence AND that the app still works, including that signed-out `/` still redirects (headers must not bypass auth) | `test:security-headers` (16/16) | Done | NFR-SEC-4 |
| 11.34 | **No sign-out control + PWA cache of authenticated HTML** — `logoutAction` existed but nothing called it, so a signed-in user could not end their session from the UI. Meanwhile the PWA service worker caches authenticated pages (`pages` NetworkFirst, 24h) to satisfy the offline requirement (G6/FR-OFF); on a shared phone the next person could be served the previous user's dashboard. Fixed: `LogoutButton` on the home header that calls `logoutAction` AND best-effort purges all caches first (never blocks sign-out). Test drives the real signout endpoint: after sign-out, `/` and `/wallets` redirect again. Also hardened two suites whose parsers broke after the button's inline SVG landed between label and value — `afterLabel` now scans 4000 chars and `Keluar` anchors after `Masuk` so it cannot match the logout button | `test:logout` (10/10) | Done | NFR-SEC-5 |
| 11.35 | **Offline feature gap (FR-OFF-2/3/4/6 P0/P1)** — PRD promised offline transaction logging with local queue + auto-sync + indicator + IndexedDB persistence, but none existed; ARCHITECTURE referenced `offline/queue.ts` that was never written. Built minimal-correct: `lib/offline/queue.ts` (raw IndexedDB, survives reload), `lib/offline/sync.ts` (client-only drain calling the real server action, oldest-first, terminal refusals dropped so the queue cannot wedge, transient failures keep the op), `OfflineIndicator` (banner + auto-drain on reconnect, hidden on auth screens), and `QuickLogSheet` now enqueues instead of erroring when `navigator.onLine` is false. FR-OFF-5 (conflict resolution / dedupe) is NOT built — a sync killed after server commit can double-create; marked in code as `ponytail` follow-up. Real browser offline E2E still needed (this test is logic + structural, IndexedDB requires a browser) | `test:offline` (15/15) | PARTIALLY DONE | FR-OFF-2,3,4,6 |
| 11.36 | **Three P0/P1 feature gaps closed: Quick-Log ergonomics + partial debt payment** — (1) FR-LOG-2: amount field now auto-focuses on open; (2) FR-LOG-7: wallet defaults to the last-used one (localStorage `kasdesk:last-wallet`), fallback first wallet; (3) FR-DBT-3: `settleDebt(id, amount?)` now records PARTIAL payments — undefined = full settle (backward compatible), rejects overpay and non-positive amounts, `isPaid` flips only when total reached, `settledAt` set only on completion. UI: new "Bayar Sebagian" modal on each open debt row (icon, input, sisa shown) | `test:debt-partial` (13/13: contract + behaviour + ownership) | Done | FR-LOG-2, FR-LOG-7, FR-DBT-3 |
